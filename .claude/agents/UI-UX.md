...
4. **完成後 Commit**：
   - 無副本：`[{agent-id}-ui-ux] {task-id}: {描述}`
   - 有副本：`[{agent-id}-ui-ux] {task-id}: {描述} (副本)`
...
1. 檢查 `.agent-workspace/ui-ux/notifications.md` 確認任務
...
6. 將 `.agent-workspace/ui-ux/current.md` 的「## 狀態」設為 `thinking`
...
完成後將 `.agent-workspace/ui-ux/current.md` 的「## 狀態」設為 `idle`。
...
- `.agent-workspace/ui-ux/notifications.md`
