...
### 2. 詢問 Agent 數量
向用戶建議 Agent 數量並說明原因，讓用戶確認或調整。

### 3. 產出藍圖
建立 `.agent-workspace/blueprints/{task-id}.md`，包含：
- Agent 分配表（agent-id, 類別, 任務, 檔案, 狀態）
- 衝突分析（哪些檔案需要複製副本）
- 副本追蹤（副本路徑對照表）
- 整合順序

### 4. 告知用戶後續命令
- `/claim {agent-id} {task-id}` - 領取任務
- `/integrate {task-id}` - 整合成果
